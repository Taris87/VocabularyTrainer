...
export const PersonalVocabulary: React.FC = () => {
  ...
  // Handle vocabulary deletion all
  const handleDeleteAll = async () => {
    ...
    try {
      await deleteAllPrivateVocabulary(user.uid);
      setVocabularies([]); // Clear the local state after deletion
    } catch (err) {
      setError("Failed to delete all vocabulary");
      console.error(err);
    }
  };
  ...
  const VocabularyForm = () => (
    <form
      ...
      className="space-y-4 mb-8 bg-white p-4 sm:p-5 rounded-lg shadow-md">
        ...
        <div className="flex flex-col sm:flex-row items-center mb-6 space-y-4 sm:space-y-0">
          {!isAdding && (
            <button
              ...
              className="md:mr-4 w-full sm:w-auto flex items-center justify-center px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-md hover:bg-indigo-700">
          ...
          <button
            ...
            className="w-full sm:w-auto flex items-center justify-center px-4 py-2 text-sm font-medium text-red-600 border border-red-600 rounded-md hover:bg-red-600 hover:text-white">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-2 sm:px-4 lg:px-6 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-[30%]">
                  German
                </th>
                <th className="px-2 sm:px-4 lg:px-6 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-[30%]">
                  English
                </th>
                <th className="hidden sm:table-cell px-2 sm:px-4 lg:px-6 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-[20%]">
                  Category
                </th>
                <th className="px-2 sm:px-4 lg:px-6 py-2 sm:py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider w-[20%]">
                  Actions
                </th>
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-gray-200">
              {vocabularies
                .sort((a: Vocabulary, b: Vocabulary) => {
                  const dateA = a.createdAt
                    ...
                    : -Infinity; // Treat null/undefined as the earliest date
              ...
              {vocabularies.length === 0 && (
                <tr>
                  <td
                    colSpan={4}
                    className="px-2 sm:px-4 lg:px-6 py-4 text-center text-gray-500 text-sm">
                    No vocabulary words added yet. Click "Add New Word" to get
                    started!
                  </td>
                </tr>
              )}
